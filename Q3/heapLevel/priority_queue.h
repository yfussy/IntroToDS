...
class priority_queue
{
  ...
  public:
    ...
    void printHeap() {
      ...
      while (index < n) {
          size_t levelNodes = 1ULL << level;       // nodes in this level
          size_t count = std::min(levelNodes, n - index); // remaining nodes
          size_t space = (1ULL << (5 - level));    // spacing for readability

          for (size_t i = 0; i < count; ++i) {
              std::cout << std::setw(space) << mData[index++] << " ";
          }
          std::cout << std::endl;
          ++level;
      }
